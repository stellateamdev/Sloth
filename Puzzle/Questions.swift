...
class QuestionsManager:NSObject {
    ...
    override init() {
        ...
        self.questions.append(question)
        self.getCorrectQuestion()
    }
    
    private func getCorrectQuestion() {
        guard let data = UserDefaults.standard.data(forKey: "questions") else {
            return
        }
        guard let questionsWonList = NSKeyedUnarchiver.unarchiveObject(with: data) as? [Question] else {
            return
        }
        for i in 0..<questionsWonList.count {
            if (questionsWonList[i].won != self.questions[i].won ) {
                self.questions[i].won = questionsWonList[i].won
            }
        }
    }
}
